$(function(){
    ...
    $('.floatBtn ul li').hover(function(){
    	...
    	if($what.css('display')=='block'){
    		$(this).css('background-color','#ee5544');
    	}
    ...
    $('.compute').click(function(event) {
    	$('.countM').animate({'left':'-300px'},400);
    });
    $('.i-close').click(function(event) {
    	$('.countM').animate({'left':'60px'},400);
		$('#calResult').hide();
    });
    $('.clo-blue').click(function(event) {
    	$('.countM').stop().animate({'left': '-970px','width': '1500px'},400);
    	$('#calList').show();
    });
    $('#backResult').click(function(event) {
    	$('.countM').stop().animate({'left': '-565px','width': '544px'},400);
    	$('#calList').hide();
    });
    /*-----右侧浮动标签 end*/
